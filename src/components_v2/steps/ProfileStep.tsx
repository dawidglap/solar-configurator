...
export default function ProfileStep() {
  ...
  // ✅ selezione cliente esistente SOLO locale
  const [selectedExistingId, setSelectedExistingId] = useState<string>("");

  // ✅ default PRIVAT se non valorizzato
  useEffect(() => {
    if (!profile.customerType) setProfile({ customerType: "private" } as any);
    if (!profile.customerStatus) setProfile({ customerStatus: "new" } as any);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const isExisting = profile.customerStatus === "existing";
  const isCompany = profile.customerType === "company";
  const companyDisabled = !isCompany;
  ...
  // ✅ layout sempre stabile (3 colonne + icone su xl)
  const topGridCols =
    "grid-cols-1 mt-4 lg:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_64px]";

  const secondGridCols =
    "grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_64px]";

  // ✅ input uguali e non full-screen (come screenshot)
  const fieldWidth = "w-[260px] max-w-full";

  // ✅ allineamento: la “riga radio” sopra ai select ha altezza fissa
  const topRadioRowHeight = "min-h-[44px]"; // regola qui se vuoi più/meno aria
  const disabledCompanyCls = companyDisabled
    ? "opacity-60 pointer-events-none"
    : "";

  return (
    <div className="w-full h-full rounded-bl-2xl text-neutral-50 shadow-xl border-l border-[#7E8B97] bg-transparent backdrop-blur-md px-6 py-4 flex flex-col">
      <div className="flex-1 space-y-6">
        ...
        <div className={`grid gap-4 items-start ${topGridCols}`}>
          ...
          <div className="space-y-2">
            ...
            {/* ✅ altezza fissa => allinea i select tra le colonne */}
            <div
              className={`flex flex-col gap-2 text-[11px] ${topRadioRowHeight}`}
            >
            ...
            {/* ✅ SEMPRE visibile (solo disabled se non existing) */}
            <select
              className={`${fieldWidth} mt-2 rounded-full bg-white/5 border-1 border-white/80 px-3 py-[5px] text-[11px] focus:outline-none focus:ring-2 focus:ring-emerald-500/70 disabled:opacity-60 disabled:pointer-events-none`}
          ...
          <div className="space-y-2">
            ...
            {/* ✅ altezza fissa => allinea i select tra le colonne */}
            <div
              className={`flex flex-wrap gap-3 text-[11px] items-start ${topRadioRowHeight}`}
            >
              <label className="inline-flex items-center gap-1">
                <input
                  type="radio"
            ...
            {/* ✅ sempre alla stessa altezza degli altri select */}
            <select
          ...
          {/* Rechtsform (SEMPRE visibile, disabled se Privat) */}
          <div className="space-y-2">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-neutral-200">
              Geschäft
            </h3>

            {/* ✅ spacer con stessa altezza dei radio sopra */}
            <div className={topRadioRowHeight} />
        ...
        <div className={`${secondGridCols} gap-6`}>
          ...
          {/* Geschäft (SEMPRE visibile; disabled se Privat) */}
          <div className={`space-y-3 ${disabledCompanyCls}`}>
            <h3 className="text-xs font-semibold uppercase tracking-wide text-neutral-200">
              Firmenadresse
            </h3>
...
function InputField({
  ...
  return (
    <label className={`block ${className ?? ""}`}>
      ...
      <input
        type="text"
        className={`w-full rounded-full bg-white/5 border-1 border-white/80 px-3 py-[5px] text-[11px] focus:outline-none focus:ring-2 focus:ring-emerald-500/70 disabled:opacity-60 disabled:pointer-events-none ${
          small ? "max-w-full" : ""
        }`}
        value={value}
        onChange={onChange}
