...
function edgeMeta(
  ...
  for (let i = 0; i < pts.length; i++) {
    ...
    const tooClose = out.some(
      (h) => dist2(h.mx, h.my, mx, my) < NEAR_V * NEAR_V,
    );
    if (!tooClose) {
...
function iconToPaths(IconComp: any): {
  ...
  const paths = children
    .filter((c: any) => c?.props?.d)
    .map((c: any) => c.props.d as string);
  return {
    paths,
    vbW: vbW || 24,
    vbH: vbH || 24,
...
function stagePxToImgPx(
  toImg: (x: number, y: number) => { x: number; y: number },
  n = 1,
) {
  const a = toImg(0, 0);
  const bx = toImg(n, 0);
...
export default function RoofShapesLayer({
  ...
  const [isTransformingRoof, setIsTransformingRoof] = useState(false);
  const finalRoofPointsRef = useRef<Record<string, Pt[]>>({});
  const cancelRoofGestureRef = useRef<(() => void) | null>(null);

  useEffect(() => {
    const onEscape = (event: KeyboardEvent) => {
      const cancel = cancelRoofGestureRef.current;
  ...
  return (
    <>
      ...
      {layers.map((r) => {
        ...
        const rotPivot = isSel
          ? (pivotPx ?? { x: bb.cx, y: bb.cy })
          : undefined;
        const rotHandlePos =
          isSel && rotPivot ? { x: bb.cx, y: bb.minY - ROT_KNOB_OFFSET } : null;
              ...
              <KonvaGroup
                ...
                onMouseEnter={(ev) => {
                  ev.target
                    .getStage()
                    ?.container()
                    ?.style.setProperty("cursor", e.cursor);
                }}
                onMouseLeave={(ev) => {
                  ev.target
                    .getStage()
          ...
          <KonvaGroup key={r.id} id={`roof-group-${r.id}`}>
            <KonvaLine
              ...
              onMouseLeave={(e) => {
                ...
                if (isSel || isCanvaGroupDrag)
                  c?.style.removeProperty("cursor");
              }}
              onMouseDown={(e) => {
                if (roofsLocked) {
                  // ⬅️ blocca ogni tipo di drag in modules
                  e.cancelBubble = true;
                  return;
                }
                e.cancelBubble = true;
                ...
                if (isCanvaGroupDrag || forceAll) {
                  ...
                  for (const l of layers)
                    groupStartPtsRef.current[l.id] = l.points.map((p) => ({
                      ...p,
                    }));
                  ...
                  const p0 = st.getPointerPosition();
                  if (!p0) return;
                ...
                idsToMove.forEach((id) => {
                  ...
                  if (roof)
                    groupStartPtsRef.current[id] = roof.points.map((p) => ({
                      ...p,
                    }));
                });
                moveGroupsRef.current = {};
                  ...
                  <KonvaGroup
                    ...
                    onMouseDown={(e) => {
                      ...
                      st.on("mousemove" + ns + " touchmove" + ns, (ev: any) => {
                        ...
                        finalRoofPointsRef.current[r.id] = rotated;
                        if (roofGroup && startPivotRef.current) {
