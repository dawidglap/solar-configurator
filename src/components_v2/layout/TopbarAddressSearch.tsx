...
function pickRoofsForBuildingV1Style(
  ...
  const bbox = (pts:{x:number;y:number}[]) => { let minX=Infinity,minY=Infinity,maxX=-Infinity,maxY=-Infinity; for (const p of pts){ if(p.x<minX)minX=p.x; if(p.y<minY)minY=p.y; if(p.x>maxX)maxX=p.x; if(p.y>maxY)maxY=p.y; } return { minX, minY, maxX, maxY }; };
  ...
  for (const p of polys) {
    ...
    const c = centroid(p.pointsPx); const d2 = (c.x - cx) ** 2 + (c.y - cy) ** 2;
...
export default function TopbarAddressSearch() {
  ...
  const startFromAddress = async (lat: number, lon: number) => {
    ...
    try {
      ...
      const { dataUrl, width: w, height: h } = await buildTiledSnapshot({
        lat, lon, zoom: ZOOM, width: 2800, height: 1800, scale: 1, tileUrl: TILE_SWISSTOPO_SAT, attribution: '© swisstopo'
      });
      ...
      const snapObj = {
        url: dataUrl, width: w, height: h, mppImage: metersPerPixel3857(lat, ZOOM),
        center: { lat, lon }, zoom: ZOOM, bbox3857: { minX: bl.x, minY: bl.y, maxX: tr.x, maxY: tr.y },
      } as const;
      ...
      setDetectedRoofs(building.map((p, i) => ({
        id: `sd_${p.id}_${i}`, points: p.pointsPx, tiltDeg: p.tiltDeg, azimuthDeg: p.azimuthDeg, source: 'sonnendach' as const
      })));
  ...
  return (
    <>
      <div
        className={`
          relative z-[1000000]  /* sopra qualunque overlay */
      ...
      >
        ...
        <div className="flex-1 [&_input]:h-7 [&_input]:w-full [&_input]:bg-transparent [&_input]:border-0 [&_input]:outline-none [&_input]:text-sm [&_input]:placeholder:text-neutral-400">
          <AddressSearchOSM
            placeholder="Adresse suchen…"
            onPick={(r) => startFromAddress(r.lat, r.lon)}
            disabled={loading}
            // se il componente supporta queste, forziamo il portale del menu
            menuPortalTarget={typeof document !== 'undefined' ? document.body : undefined as any}
            dropdownClassName="z-[1000000]"
            containerClassName="z-[1000000]"
            inputClassName="!bg-transparent !border-0 !outline-none !shadow-none"
          />
        </div>
      ...
      {/* Fallback globale: qualunque libreria (autosuggest, select, leaflet, google) */}
      <style>{`
