...
// import GridCoverageControl from '../modules/GridCoverageControl'; // ← rimosso: non usato
...
export default function ModulesPanel() {
  ...
  // — le seguenti tre sono state rimosse dall’UI, ma lasciamo lo stato intatto nello store
  ...
  return (
    <div className="w-full max-w-[240px] space-y-3 p-2">
      ...
      */}
      ...
      {/* AUSRICHTUNG — rimosso selettore portrait/landscape */}

      {/* MODUL — rimosso dropdown moduli */}
      ...
      {/* Allineamento/ancoraggi raster (manteniamo il componente) */}
      

      {/* Rotazione raster (manteniamo il componente) */}
...
*/}
        {/* In Module umwandeln – mantiene la stessa logica della preview */}
         {/* 
        <button
          disabled={disabled}
          onClick={() => {
            ...
            // usa la stessa identica logica della preview (angolo base)
            const eavesCanvasDeg = -(roof.azimuthDeg ?? 0) + 90;

            // angolo dell’edge più lungo (deg)
            const polyDeg = (() => {
              let best = 0, len2 = -1;
            ...
            const diff180 = diff > 180 ? 360 - diff : diff;
            const baseCanvasDeg = diff180 > 5 ? polyDeg : eavesCanvasDeg;
            const gridCanvasDeg = baseCanvasDeg + (m.gridAngleDeg || 0);

            const rects = computeAutoLayoutRects({
              polygon: roof.points,
              mppImage: snapshot.mppImage!,
              azimuthDeg: gridCanvasDeg,   // match alla preview
