...
type LeadRow = {
  ...
  source: string;
  owner: string;
  status: "konvertiert" | "Junk" | "gelöscht";
};
...
export default function LeadsPage() {
    ...
    () => [
      {
        id: "1",
        name: "Emil Egger",
        plz: "9435 Heerbrugg SG",
        phone: "076 300 99 89",
        email: "edin.mustafic@gmail.com",
        source: "www.grünerwohnen.ch",
        owner: "Edin Mustafic",
        status: "konvertiert",
      },
      {
  ...
  // per mostrare un menu aperto come nello screenshot
  const [openMenuId, setOpenMenuId] = useState<string | null>("3");
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <div className="grid grid-cols-[1.4fr_1.8fr_1.4fr_2.1fr_2.0fr_1.4fr_1.0fr_44px] items-end gap-4 text-white/85">
          <HeaderCell>Name</HeaderCell>
          <HeaderCell>PLZ</HeaderCell>
          <HeaderCell>Telefon</HeaderCell>
          <HeaderCell>E-Mail</HeaderCell>
          <HeaderCell>Quelle</HeaderCell>
...
function statusClass(status: LeadRow["status"]) {
  if (status === "konvertiert") return "text-emerald-300";
  if (status === "Junk") return "text-rose-300";
  return "text-white/85"; // gelöscht
}
...
function LeadRowItem({
  ...
  return (
    <div className="relative">
      <div
        className={[
          "grid grid-cols-[1.4fr_1.8fr_1.4fr_2.1fr_2.0fr_1.4fr_1.0fr_44px] items-center gap-4",
      ...
      >
        <div className="col-span-full rounded-xl border border-white/15 px-5 py-5">
          <div className="grid grid-cols-[1.4fr_1.8fr_1.4fr_2.1fr_2.0fr_1.4fr_1.0fr_44px] items-center gap-4">
            ...
            <Cell className="text-white/85">{row.email}</Cell>
            <Cell className="text-white/85">{row.source}</Cell>
            <Cell className="text-white/85">{row.owner}</Cell>
            <Cell className={statusClass(row.status)}>{row.status}</Cell>
